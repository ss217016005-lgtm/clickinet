...
try { 
    if (fs.existsSync(DB_PATH)) {
        ...
        if (data.trim() !== '') {
            ...
            db = { ...db, ...parsed }; 
            if(!db.analytics) db.analytics = { portal: 0, audience: 0, admin: 0 }; 
        }
    }
} catch(e) {}
...
let rooms = {}; // חדרים של קליקינט
let callToRoom = {}; // שיחות של קליקינט

let arenaRooms = {}; // חדרים של הזירה המשפחתית
let arenaCallToRoom = {}; // שיחות של הזירה המשפחתית
...
app.get('/arena', (req, res) => res.sendFile(__dirname + '/arena.html')); // 🆕 הראוט של הזירה!
...
app.all('/api/arena', (req, res) => {
    ...
    try {
        ...
        console.log(`⚔️ שיחה לזירה! טלפון: ${phone} | הקשה: ${val} | חדר: ${arenaCallToRoom[callId] || 'טרם נבחר'}`);
        ...
        if (!roomId) {
            if (val !== undefined && val !== '') {
                ...
                if (!arenaRooms[val]) arenaRooms[val] = { players: {}, phase: 'lobby' }; // יצירת חדר זירה אם לא קיים
                return res.send(`id_list_message=t-מחובר לזירה&read=t-ממתין=val_1,no,10,1,10,no,no`); // ממתין להקשה עד 10 תווים!
        ...
        // השחקן הקיש מספר! (בזירה הם מקישים 1 עד 10)
...
io.on('connection', (socket) => {
    // === סוקטים קיימים של מנהל על וקליקינט ===
    ...
    socket.on('arenaSetPhase', data => {
        if(arenaRooms[data.roomId]) arenaRooms[data.roomId].phase = data.phase;
    });

    socket.on('arenaUpdateScores', data => {
        if(arenaRooms[data.roomId]) {
            arenaRooms[data.roomId].players = data.players; // סנכרון הניקוד לשרת
        }
    });

    socket.on('arenaUpdatePlayerName', data => {
        if(arenaRooms[data.roomId]) {
            db.phonebooks[data.roomId] = db.phonebooks[data.roomId] || {};
            db.phonebooks[data.roomId][data.phone] = data.name;
            saveDB();
            if(arenaRooms[data.roomId].players[data.phone]) {
                arenaRooms[data.roomId].players[data.phone].name = data.name;
            }
        }
    });
});
...
http.listen(PORT, '0.0.0.0', () => console.log("=== Clickinet V61.0 (Arena Multi-Game API) is ONLINE ==="));
