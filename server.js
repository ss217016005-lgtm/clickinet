    ...
    try {
        ...
        if (!roomId) {
            if (val !== undefined && val !== '') {
                ...
                callToRoom[callId] = val; 
                const room = getRoom(val);
                
                // ⚡️ הרישום המיידי - טריוויה ⚡️
                if (!room.activePlayers[phone]) { 
                    room.activePlayers[phone] = { name: db.phonebooks[val]?.[phone] || "שחקן חדש", score: 0, lastAnswered: -1, ping: 0, streak: 0, lastBreakdown: "" }; 
                    io.to(val).emit('updateLeaderboard', room.activePlayers); 
                }
    ...
    try {
        ...
        if (!roomId) {
            if (val !== undefined && val !== '') {
                ...
                // ⚡️ הרישום המיידי - זירה ⚡️
                const room = arenaRooms[val];
                if (!room.players[phone]) { 
                    room.players[phone] = { name: db.phonebooks[val]?.[phone] || "שחקן חדש", phone: phone, score: 0, currentVote: 0 }; 
                    io.to('arena_'+val).emit('arenaUpdatePlayers', room.players); 
                }
...
http.listen(PORT, '0.0.0.0', () => console.log("=== Clickinet V68.0 (Instant Registration & No-Limit Arena) is ONLINE ==="));
